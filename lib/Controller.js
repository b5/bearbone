...
var Controller = function (options) {
  ...
  if (this.checkCompletions()) {
    ...
    console.log(self._completer.applicationPrefix());
    this.addCompletions();
  }
}
...
Utils.extend( Controller.prototype, {
  ...
  addCompletions : function(){
    var self = this;
    self.get('all', function(models){
      models.forEach(function(model){
        if (model.id) {
          for (var key in self.completions) {
            var val = self.completions[key];
            if (model[key]) { self._completer.addCompletions( self._completerPrefix() + model[key], model.id, val); }
          }
        }
      });
    });
  ...
  autocomplete : function (query, count, callback) {
    ...
    self._completer.search(self._completerPrefix() + query, count, function(err,results){
      if (err) { callback(false); console.log(err); }
      if (results.length) {
        results.forEach(function(r,i){ results[i] = parseInt(r.split(':')[0], 10); });
        results = _.uniq(results);
        self.model.read(results, function(models){
