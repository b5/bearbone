...
var Relationships = Protocol.extend({
  ...
  cb : function (err, res, callback) {
	...
	get : function (id, relationshipName, callback) {
		...
		db.setMembers(self.nameSpace(id) + '.' + Model.name, function(err,relationships){
	...
	read : function (id, relationshipName, callback) {
		var self = this;
		self.get(id, relationshipName, function (err, ids) {
			if (err) return self.cb(err,undefined, callback);
			self.relationships[relationshipName].model.read(ids, function(err, relationships){
	...
	getSorted :  function (id, relationshipName, sortName, number, reverse, callback) {
		...
		var self = this;
		if (reverse) {
			db.sortedSetRevRange(self._relationshipSortedSetKey(id, relationshipName, sortName), 0, number, callback);
		} else {
			db.sortedSetRange(self._relationshipSortedSetKey(id, relationshipName, sortName), 0, number, callback);
		}
	},
	...
	deleteRelationshipObjects : function (id, relationshipName, callback) {
		var self = this, count;
		self.getRelationships(id, relationshipName, function(err,refIds){
	...
	_addRelationshipAttributes : function () {
		...
		for (var refName in this.relationships) {
			...
			if (relationship.countAttribute) {
				var countAttributeName = self._relationshipCountName(refName);
	...
	addReference : function (name, relationship, model, options) {
		...
		if (relationship.countAttribute) { 
			tasks++;
			db.hashIncrBy(self.nameSpace(id), self._relationshipCountName(name), 1, done); 
		}

		if (relationship.currentAttribute){
			tasks++;
			var hash = {};
			hash[relationship.currentAttribute] = id;
		...
		db.setAdd(self.nameSpace(id) + '.' + relationship.model.name, model.id, function(err){
			if (err) { return; }
			done();
		});
	},
	removeReference : function (name, relationship, model, options) {
		...
		db.setRemove(self.nameSpace(id) + '.' + relationship.model.name, model.id, function(err){
			if (err) { return; }
			done();
		});
	},
	referenceUpdated : function (name, relationship, model, oldModel, options) {
		...
		var self = this
			, id = model[relationship.key]
			, oldId = oldModel[relationship.key];
