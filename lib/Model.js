...
utils.extend( Model.prototype, {
	...
	readReferences : function (id, referenceName, callback) {
		var self = this;
		this.getReferences(id, referenceName, function (refIds) {
			if (!refIds) return self.cb(refIds, callback);
			if (!refIds.length) return self.cb([],callback);

			self.references[referenceName].model.read(refIds, function(references){
				self.cb(references, callback);
			});
		});
	},
	...
	addReference : function (name, reference, model) {
		...
		self.db.setAdd(self.nameSpace(id) + reference.model.name, model.id, function(){
			if (typeof reference.added === 'function') reference.added(model);
	...
	removeReference : function (name, reference, model) {
		...
		self.db.setRemove(self.nameSpace(id) + reference.model.name, model.id, function(){
			if (typeof reference.removed === 'function') reference.removed(model);
