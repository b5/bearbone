	...
	validate: function(attrs, options) {
		...
		for (var attr in attrs) {
			...
			if (self._modelType(attrType)) {
				...
				if (!attrs[attr]._type) { return new Err.Model("Invalid Model for " + self.name + " attribute: " + attr); }
				if (attrType != attrs[attr]._type) { return new Err.Model(self.name  + ":" + attr + " type Mismatch. expected: " + attrType + ", received: " + attrs[attr]._type ); }
