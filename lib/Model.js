...
var Model = function (options) {
	...
	if (self.references) {
		...
		self._addReferenceCountAttributes.call(self);
	}
...
utils.extend( Model.prototype, {
	...
	create : function (attrs, callback) {
		var self = this
			, valid = this.validate(attrs);
		if (!valid || valid instanceof Err.Model){ return self.cb(valid, false, callback); }
	...
	update : function (attrs, options, callback) {
		var self = this
			, valid = self.validate(attrs, { update : true });
	...
	_addReferenceCountAttributes : function () {
		var self = this;
		for (var refName in this.references) {
			var reference = this.references[refName];
