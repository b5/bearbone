...
utils.extend( Model.prototype, {
	...
	// sortedSets - an array of reference attributes that the model should hold a sorted set of.
	...
	getSortedReferences :  function (id, referenceName, sortName, number, reverse, callback) {
		if (arguments.length === 4) { callback = number; number = 5; reverse = false; }
		else if (arguments.length === 5) { callback = reverse; reverse = false; }
		var self = this;
		if (reverse) {
			self.db.sortedSetRevRange(self._referenceSortedSetKey(id, referenceName, sortName), 0, number, callback);
		} else {
			self.db.sortedSetRange(self._referenceSortedSetKey(id, referenceName, sortName), 0, number, callback);
		}
	},
	readSortedReferences : function (id, referenceName, sortName, number, reverse, callback) {
		if (arguments.length === 4) { callback = number; number = 5; reverse = false;}
		else if (arguments.length === 5) { callback = reverse; reverse = false; }
		var self = this;
		
		self.getSortedReferences(id, referenceName, sortName, number, reverse, function(err,ids){
			if (err) { return self.cb(err, null, callback); }
			self.references[referenceName].model.read(ids,callback);
		});
	},
	...
	_referenceSortedSetKey : function(id, referenceName, sortName) { return this.nameSpace(id) + '.' + referenceName + '.sorted.' + sortName; },
	_addReferenceAttributes : function () {
		...
		for (var refName in this.references) {
			...
			if (reference.currentAttribute) {
				self.attributes[reference.currentAttribute] || (self.attributes[reference.currentAttribute] = ["number"])
			}
		}
	},
	addReference : function (name, reference, model, options) {
		...
		if (reference.currentAttribute){
			tasks++;
			var hash = {};
			hash[reference.currentAttribute] = id;
			self.db.setHash(self.nameSpace(id),hash, done);
		}

		if (reference.sortedSets instanceof Array) {
			tasks++;
			count = reference.sortedSets.length;
			reference.sortedSets.forEach(function(sortedAttributeName){
				if (model[sortedAttributeName]) {
					self.db.sortedSetAdd(self._referenceSortedSetKey(id, name, sortedAttributeName),model[sortedAttributeName],model.id, function(err,res){
						if (--count === 0 ) { done(); }
					});
				}
			});
	...
	removeReference : function (name, reference, model) {
		...
		if (reference.currentAttribute){
			tasks++;
			self.db.hashDelete(self.nameSpace(id),reference.currentAttribute, done);
		}
		...
		if (reference.sortedSets instanceof Array) {
			...
			reference.sortedSets.forEach(function(sortedAttributeName){
				if (model[sortedAttributeName]) {
					self.db.sortedSetRemove(self._referenceSortedSetKey(id, name, sortedAttributeName),model[sortedAttributeName],model.id, function(err,res){
						if (--count === 0 ) { done(); }
					});
				}
			});
	...
	_referencesCascadeDelete : function (id, callback) {
		var self = this, refName, reference, tasks = 1;
		if (!self.references) { return self.cb(null,false,callback); }
		if (!Object.keys(self.references).length) { return self.cb(null,false, callback); }
